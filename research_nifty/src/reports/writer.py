...
def build_markdown_report(state: ResearchState) -> str:
    news_brief = "\n".join(
        [f"- [{i.timestamp.date()}] {i.headline} ({i.source})" for i in state.sentiment_report.items[:5]]
    ) or "- No recent headlines captured"
...
- MACD / Signal: {state.technical_metrics.macd:.3f} / {state.technical_metrics.macd_signal:.3f}
- Bollinger (U/L): {state.technical_metrics.bb_upper:.2f} / {state.technical_metrics.bb_lower:.2f}
- Stoch (K/D): {state.technical_metrics.stochastic_k:.1f} / {state.technical_metrics.stochastic_d:.1f}
- Support / Resistance: {state.technical_metrics.support_level} / {state.technical_metrics.resistance_level}
...
- Approx entry date: {state.risk_report.estimated_entry_date}
...
def build_json_summary(state: ResearchState) -> dict:
    return {
        ...
        "estimated_entry_date": state.risk_report.estimated_entry_date,
        ...
        "news_brief": [f"{i.timestamp.date()} | {i.headline}" for i in state.sentiment_report.items[:5]],
